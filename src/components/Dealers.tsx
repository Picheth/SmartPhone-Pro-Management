...
import { Briefcase, ArrowRightLeft, PlusCircle, ShieldCheck } from 'lucide-react';
...
export default function Dealers() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'dealers'), s => 
      setDealers(s.docs.map(d => ({ id: d.id, ...d.data() } as Dealer))),
  ...
  const addPartner = async () => {
    ...
    await addDoc(collection(db, 'dealers'), newP);
    setNewP({ code: '', name: '' });
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800 tracking-tight">Dealer Network</h2>
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Manage B2B Partnerships</p>
        </div>
      </div>
      ...
      <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm space-y-6">
        <div className="flex items-center gap-2 mb-2">
           ...
           <h3 className="text-sm font-bold text-slate-800 uppercase tracking-tight">Register New Dealer</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="space-y-1.5">
            <label className="text-[10px] uppercase font-bold text-slate-400 tracking-widest pl-1">Dealer Business Name</label>
            <input 
              placeholder="e.g. KY VIP Outlet"
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] uppercase font-bold text-slate-400 tracking-widest pl-1">Dealer Code</label>
            <input 
        ...
        {dealers.map((p) => (
          <div key={p.id} className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all group relative overflow-hidden">
            <div className="absolute top-0 right-0 w-24 h-24 bg-slate-50/50 rounded-bl-[100px] flex items-start justify-end p-4">
               <ShieldCheck className="w-8 h-8 text-slate-100 group-hover:text-blue-50 group-hover:scale-125 transition-all" />
            </div>
